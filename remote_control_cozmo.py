...
def handle_index_page():
    ...
    <html>
        <head>
            ...
            <script src="''' + get_url("blockly_compressed.js") +  '''"></script>
            <script src="''' + get_url("blocks_compressed.js") +  '''"></script>
            <script src="''' + get_url("python_compressed.js") +  '''"></script>
            <script src="''' + get_url("en.js") + '''"></script>
            <script src="''' + get_url("my01.js") + '''"></script>          
            <script src="''' + get_url("hint01.js") + '''"></script>
            ...
            <xml id="toolbox" style="display: none">
                ...
                <block type="indirect_hint"></block>       
            </xml>
        ...
        <!--
            ...
            <table>
                <tr>
                    ...
                    <h1>test headline</h1>
                    //<button id="headlightId" onClick=onHeadlightButtonClicked(this) style="font-size: 14px">Default</button>
                    -->
                    <button id="run_blockly" onClick=run_blockly_code(this) style="font-size: 14px">Run Code</button>
                    <div id="blocklyDiv" style="height: 880px; width: 1400px; border-style: solid;"></div>
                    </td>
            ...
            <script type="text/javascript">
                ...
                function run_blockly_code(button)
                {
                    postHttpRequest('blockly_greeting', {'code':Blockly.Python.workspaceToCode(workspace)});
                }
...
@flask_app.route('/blockly_greeting', methods=['POST'])
def blockly_greeting():
    '''Called from javascript when the run button pressed'''
    print('new version blockly_called')
    message = json.loads(request.data.decode("utf-8"))
    if remote_control_cozmo:
        # print(message)
        exec(message['code'])
...
'''@flask_app.route('/blockly_indirect_hint', methods=['POST'])'''


@flask_app.route('/blockly_retrive_test', methods=['POST'])
def blockly_retrive_test():
    print('new version blockly_called')
    message = json.loads(request.data.decode('utf-8'))
    if remote_control_cozmo:
        # print(message)
